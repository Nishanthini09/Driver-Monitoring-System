...
class DrowsinessDetector:
    ...
    def analyze(self, frame):
        ...
        if results.multi_face_landmarks:
            landmarks = results.multi_face_landmarks[0]  # <--- This line fixes the issue
            ...
            face_coords = [(int(lm.x * w), int(lm.y * h)) for lm in landmarks.landmark]
            x_vals = [pt[0] for pt in face_coords]
            y_vals = [pt[1] for pt in face_coords]
            cv2.rectangle(frame, (min(x_vals), min(y_vals)), (max(x_vals), max(y_vals)), (255, 255, 0), 2)
